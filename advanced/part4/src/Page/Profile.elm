...
init session username =
    ...
    , Cmd.batch
        ...
        , fetchFeed model.session (currentUsername model.author) defaultFeedTab 1
...
update msg model =
    case msg of
        ...
        ClickedTab tab ->
            ( { model | feedTab = tab }
            , fetchFeed model.session (currentUsername model.author) tab 1
            )

        ClickedFeedPage page ->
            ( { model | feedPage = page }
            , fetchFeed model.session (currentUsername model.author) model.feedTab page
            )
